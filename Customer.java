/**
 * The standard customer (Thread class), which has two action pay and drives.
...
public class Customer implements Runnable {
	 ...
	 */
	public Customer(int customerID,int maxPetrolUnits, GasStation gasStationController, GasStationEvent gasStationEvent) {
		this.customerID = customerID;
		this.standard = true;			//true(standard customer), false(E10 customer)
	 ...
	 */
	public synchronized int pay(int howManyUnits) throws InterruptedException {
		...
		return this.gasStationController.getCashier().pay(this.customerID, howManyUnits); 
	}
				...
				if(randomActionChoicce == 0) {
					...
					if(randomUnits != 0) {
						...
						if(randomUnits <= this.maxPetrolUnits-customerGasUnits) {
							int pumpID = this.pay(randomUnits);
							gasStationEvent.sleepEvents();//for testing
							gasStationController.getPumps()[pumpID].start(customerID, randomUnits);
							gasStationEvent.sleepEvents();//for testing
							gasStationController.getPumps()[pumpID].gas(customerID, randomUnits,gasStationController);
							gasStationEvent.sleepEvents();//for testing
						}
					}
				} 
